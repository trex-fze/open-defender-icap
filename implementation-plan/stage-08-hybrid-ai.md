...
| S8-T5 | Provider-level metrics & alerts | SRE | S8-T1–T3 | 🔄 | Per-provider counters/latency histograms (in progress) + Prometheus rules TBD |
| S8-T6 | Compose overlays & docs for LM Studio/Ollama | DevOps | S8-T2 | ⬜ | Provide optional services + instructions for local inference nodes |
| S8-T7 | Security/perf validation (prompt injection, fallback load) | Security/Perf Eng | S8-T2–T4 | ⬜ | Extend `tests/security` + k6 to cover provider failover |
| S8-T8 | Evidence & runbooks (Stage 8) | TPM | S8-T1–S8-T7 | ⬜ | Create `docs/evidence/stage08-hybrid-ai.md` once validation completes |
