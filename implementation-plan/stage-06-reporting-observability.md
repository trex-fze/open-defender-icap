...
| S6-T5 | Implement report APIs + CLI helpers | Backend Eng | S5 API | ✅ | Admin API now exposes `/api/v1/reporting/traffic` (ES-backed) and `odctl report traffic` consumes it |
| S6-T6 | Evidence capture (screenshots, logs) | QA/SOC | S6-T3 | ✅ | `docs/evidence/stage06-observability.md` + sample traffic report JSON guide |
| S6-T7 | Integration tests for ingestion/dashboards/alerts | QA/SRE | S6-T1–T5 | ✅ | `tests/stage06_ingest.sh` smoke test validates ingester→ES→API + alert instructions |
