...
1. Implement Stage 21 stream consumer-group migration (`XREADGROUP` + ACK/claim) with restart simulation tests.
2. Implement Stage 22 cursor parity for remaining policy/reporting list APIs.
3. Keep Stage 18 reliability gate green in routine regression runs (`RUNS=10`).
4. Keep Stage 20 diagnostics collector/runbook path validated in on-call drills.
