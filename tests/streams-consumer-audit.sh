...
docker compose -f "$COMPOSE_FILE" exec -T redis redis-cli XINFO GROUPS classification-jobs >"$OUT_DIR/classification-jobs-groups.txt" 2>&1 || true
docker compose -f "$COMPOSE_FILE" exec -T redis redis-cli XINFO GROUPS page-fetch-jobs >"$OUT_DIR/page-fetch-jobs-groups.txt" 2>&1 || true
