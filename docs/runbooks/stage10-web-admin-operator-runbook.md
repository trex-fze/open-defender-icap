...
- **Dashboard platform health panel reports degraded/down components**: call `GET /api/v1/ops/platform-health?refresh=true` and inspect `errors[]` plus per-component `detail`/`status`. Verify component endpoint overrides (`OD_EVENT_INGESTER_URL`, `OD_PAGE_FETCH_METRICS_URL`, `OD_RECLASS_METRICS_URL`, `OD_ICAP_METRICS_URL`, `OD_CRAWL4AI_HEALTH_URL`, `OD_KIBANA_STATUS_URL`) and probe controls (`OD_OPS_HEALTH_ENABLED`, `OD_OPS_HEALTH_TTL_SECS`, `OD_OPS_HEALTH_TIMEOUT_MS`) match your deployment topology. For Elasticsearch `HTTP 401/403`, verify `OD_REPORTING_ELASTIC_API_KEY` or `OD_REPORTING_ELASTIC_USERNAME` + `OD_REPORTING_ELASTIC_PASSWORD` are configured correctly.
