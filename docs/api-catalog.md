...
| `GET` | `/api/v1/classifications` | List classified and/or unclassified keys for management UI. | `policy-viewer`; cursor pagination with query `state=all|classified|unclassified`, `q`, `limit`, `cursor`; returns unified state/category/action rows including historical `recommended_action`, current `effective_action`/`effective_decision_source`, and normalized `fallback_provenance[]` chips for fallback metadata on classified rows. |
