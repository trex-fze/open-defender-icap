...
| `GET` | `/api/v1/classifications` | List classified and/or unclassified keys for management UI. | `policy-viewer`; query `state=all|classified|unclassified`, `q`, `limit`; returns unified state/category/action rows including historical `recommended_action` plus current `effective_action` and `effective_decision_source` for classified rows. |
