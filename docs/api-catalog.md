...
| --- | --- | --- | --- |
| `GET` | `/api/v1/reporting/traffic` | Elastic-powered traffic summary with inferred-action/domain/category fallbacks when structured fields are sparse. | `range`, `top_n`, `bucket`; response includes `bucket_interval` and `timezone`. |
...
| `GET` | `/api/v1/reporting/dashboard` | Rich dashboard analytics with client count, bandwidth, usage trend buckets, top domains/categories, blocked domains, blocked requesters (`client.ip`), and coverage metrics. | `range`, `top_n`, `bucket`; returns `bucket_interval`, `timezone`, `overview`, `hourly_usage`, `top_domains`, mapped `top_categories` (plus optional `top_categories_event`), `top_blocked_domains`, `top_blocked_requesters`, `top_clients_by_bandwidth`, and `coverage` (`total_docs`, `client_ip_docs`, `domain_docs`, `category_docs`, `category_mapped_domain_docs`, `category_mapped_ratio`, `network_bytes_docs`). |
...
| `GET` | `/api/v1/reporting/ops-llm-series` | Prometheus range-query telemetry for dashboard LLM outcomes graph (per-provider success/failures/timeouts/non-retryable-400 over time). | `range`; returns `{ range, source, step_seconds, providers, errors }` where each provider entry includes `{ provider, success[], failures[], timeouts[], non_retryable_400[] }` and each point is `{ ts_ms, value }`. Degrades to `partial`/`unavailable` when one or more Prometheus queries fail. |
| `GET` | `/api/v1/ops/platform-health` | Aggregated component health for dashboard operations view (core services, workers, datastores, observability endpoints). | Optional `refresh=true` to bypass cache; returns `{ source, checked_at_ms, overall_status, summary, components[], errors[] }` where each component includes status/latency/detail and sanitized endpoint metadata. |
...
| `GET` | `/api/v1/cli-logs` | Retrieve CLI audit log entries. | Cursor pagination; query supports `operator_id`, `limit`, `cursor`; response is `{ data, meta }` with directional `next_cursor`/`prev_cursor`. |
...
| `POST` | `/api/v1/classifications/:normalized_key/metadata-classify` | Queue metadata-only classification for a pending site with a preferred LLM provider. | `policy-editor`; body `{ provider_name?, reason? }`; enqueues a `requires_content=false` classification job, updates pending status to `queued_manual_metadata`, and applies normal failover policy if the preferred provider fails. Returns `{ normalized_key, status, provider_name? }`. |
...
| `GET` | `/api/v1/classifications` | List classified and/or unclassified keys for management UI. | `policy-viewer`; cursor pagination with query `state=all|classified|unclassified`, `q`, `limit`, `cursor`; response meta includes directional `next_cursor`/`prev_cursor`. Returns unified state/category/action rows including historical `recommended_action`, current `effective_action`/`effective_decision_source`, and normalized `fallback_provenance[]` chips for fallback metadata on classified rows. |
