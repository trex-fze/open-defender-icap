...
4. **Run migrations**: for the shared-DB default in `.env.example`, use `odctl migrate run admin`; use `odctl migrate run all` only when admin and policy databases are separate. Before startup in local/hybrid auth mode, set a strong `OD_LOCAL_AUTH_JWT_SECRET` in root `.env` (for example from `openssl rand -base64 48`).
...
| `odctl smoke --profile compose` | Send sample ICAP REQMOD to adaptor using compose defaults | Prints ICAP status line and response payload summary. |
...
Config file location: `~/.odctl/config` (YAML/JSON) storing API endpoints & tokens. Example commands: `odctl smoke --profile compose`, `OD_POLICY_URL=http://localhost:19010 OD_ADMIN_TOKEN=secret odctl policy reload`, `OD_ADMIN_TOKEN=secret odctl policy simulate request.json`.
...
- Timezone baseline defaults to `OD_TIMEZONE=Asia/Dubai`; dashboard bucket aggregation uses `OD_REPORTING_TIMEZONE` (defaults to same value) and displays bucket timezone explicitly.
- Routes: Dashboard (Elastic analytics + Prometheus operations telemetry overlays including provider-level LLM outcome series), Investigations, Policies (+ draft create/publish), **Pending Sites** (manual classification with category/subcategory and policy-computed action; per-row metadata-only classify with preferred provider selection and fallback-aware queueing; per-row pending delete plus guarded delete-all queue cleanup; subdomain inputs auto-promote to canonical domain key), **Classifications** (classified/unclassified CRUD management with both `Effective Action` and `Recorded Action` columns), Allow / Deny list (domain + subdomain overrides), Taxonomy (canonical listing with activation toggles and save/reset), Diagnostics (Page Content + Cache tools), Settings (RBAC + CLI audit logs + Classification Exchange import/export/flush + Allow/Deny Exchange line-by-line import/export).
- Dashboard analytics: live graphs for unique clients (`client.ip`), total bandwidth, usage trend buckets (range-dependent interval + explicit timezone), frequently accessed domains, blocked domains, and top blocked requesters by `client.ip`.
- Dashboard range selector: `1m`, `5m`, `15m`, `1h`, `6h`, `24h`, `7d`, `30d`.
- LLM Outcomes panel: per-provider time-series for success, failures, timeouts, and non-retryable HTTP 400 spikes with provider scope selector (`All providers` or a specific provider).
...
- **Smoke stack**: `make compose-smoke` (preferred) runs `up -d --build`, executes `smoke-tests`, then tears down. Direct equivalent: `docker compose --env-file .env -f deploy/docker/docker-compose.smoke.yml up -d --build && docker compose --env-file .env -f deploy/docker/docker-compose.smoke.yml run --rm smoke-tests && docker compose --env-file .env -f deploy/docker/docker-compose.smoke.yml down`.
- **CI/integration**: `COMPOSE_PROFILES=dev make compose-test` (preferred) runs the test overlay smoke harness with profile-gated services available.
