...
- Routes: Dashboard, Investigations, Policies (+ draft create/publish), **Pending Sites** (manual classification with category/subcategory and policy-computed action; per-row pending delete plus guarded delete-all queue cleanup; subdomain inputs auto-promote to canonical domain key), **Classifications** (classified/unclassified CRUD management with both `Effective Action` and `Recorded Action` columns), Allow / Deny list (domain + subdomain overrides), Taxonomy (read-only canonical listing with checkbox activation toggles), Page Content diagnostics, Cache diagnostics, Settings (RBAC + CLI audit logs + classification exchange import/export/flush).
