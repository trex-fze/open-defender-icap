...
- Page fetch failures are persisted in `page_contents` with terminal `fetch_status`/`fetch_reason` values (`failed`, `blocked`, `unsupported`) so no-content progression has durable evidence. `blocked` is reserved for explicit anti-bot/access-denied outcomes, while structural/minimal-content non-page endpoints are recorded as `unsupported` (for example `no_content_endpoint`, `asset_endpoint`). Candidate hosts are DNS-prefiltered before Crawl4AI invocation; when all candidates fail DNS preflight, page-fetcher persists terminal `unsupported:dns_unresolvable` instead of repeated Crawl4AI DNS failures. Fetch records also persist targeting context (`source_url`, `resolved_url`, `attempt_summary`) so operators can audit candidate URL selection and empty-content outcomes.
- LLM metadata-only fallback threshold (`metadata_only_fetch_failure_threshold`) uses persisted terminal fetch history (recommended default `1` for pending convergence); recommended defaults are `content_required_mode=auto`, `metadata_only_allowed_for=all`, and `metadata_only_requeue_for_content=false`.
